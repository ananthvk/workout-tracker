...
const routes: FastifyPluginAsync = async (fastify, _): Promise<void> => {
  ...
  fastify.post(
    ...
    async (
      ...
      reply.send(
        await createUser(fastify, request.body.email, request.body.password),
      );
    },
  );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      reply.send(
        await getUserById(fastify, (request.user as AuthTokenDecoded).id),
      );
    },
  );
  ...
  fastify.post(
    ...
    async (
      ...
      const token = await reply.jwtSign(
        ...
        { expiresIn: fastify.tokenExpiresIn },
      );
      return { token };
    },
  );
